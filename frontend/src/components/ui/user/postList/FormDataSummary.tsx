...
const LegacyFormDataSummary = ({
  ...
  if (post.type === "FIND_PLAYER") {
    ...
    const level = (fd.playerRequirement as { level?: string } | undefined)
      ?.level;
    const customLevel = (
      fd.playerRequirement as { customLevel?: string | null } | undefined
    )?.customLevel;
    ...
    const slots = (fd.playerRequirement as { slotsNeeded?: number } | undefined)
      ?.slotsNeeded;
  ...
  if (post.type === "TOURNAMENT") {
    ...
    if (Array.isArray(categories))
      right.push(`Hạng mục: ${categories.join(", ")}`);
