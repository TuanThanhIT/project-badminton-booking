...
const EmployeeLoginPage = () => {
  ...
  const {
    ...
    defaultValues: { username: "employee1", password: "123456789" },
    mode: "onChange",
  });

  const onSubmit = async (data: formLogin) => {
    try {
      const res = await authService.loginEmployeeService(data);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 bg-white border border-gray-300 rounded-2xl w-full max-w-4xl">
        ...
        >
          ...
          <p className="text-center text-gray-500 text-sm mt-3">
            Tài khoản được cấp bởi quản lý
          </p>
        </form>
      </div>
    </div>
