      ...
      <div className="mx-auto max-w-[1500px] space-y-4">
        <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
            <div>
              <p className="text-sm font-bold text-sky-700">Vận hành ca làm</p>
        ...
        {currentWorkShift && (!isCashier || !hasCheckedIn) && (
          <section className="rounded-3xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-800">
            Chỉ thu ngân đã check-in và nhập tiền đầu ca mới được quản lý giờ
            làm của nhân viên trong ca.
          </section>
        )}
              ...
              <table className="min-w-[980px] w-full text-left text-sm">
                <thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase text-slate-500">
                  <tr>
                    ...
                    <th className="px-4 py-3">Tỷ lệ hoàn thành</th>
                    <th className="px-4 py-3">Lương ca</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {shiftAssignments.map((assignment) => {
                    const form = forms[assignment.assignmentId];
                    return (
                      <tr key={assignment.assignmentId} className="align-top">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-bold text-slate-600">
                                {assignment.employee?.username || "Nhân viên"}
                              </p>
                        ...
                        <td className="px-4 py-3">
                          <span className="inline-flex h-10 min-w-24 items-center rounded-xl bg-slate-100 px-3 font-bold text-slate-800">
                            {formatCompletionRate(assignment.completionRate)}
                          </span>
                        </td>
                        <td className="px-4 py-3">
          ...
          !currentWorkShift.checkOut && (
            <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
              <form
                ...
                className="grid gap-4 lg:grid-cols-[1fr_220px_170px]"
              >
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Nhập checkout cho nhân viên trước, sau đó chốt tiền cuối ca.
                  </p>
                </div>
        ...
        {currentWorkShift?.checkOut && (
          <section className="rounded-3xl border border-emerald-200 bg-emerald-50 p-5 text-sm text-emerald-800">
            <div className="flex items-center gap-2 font-bold">
              ...
              Ca đã checkout lúc {formatDateTime(currentWorkShift.checkOut)}
            </div>
          </section>
        )}
        ...
        <section className="grid gap-3 md:grid-cols-3">
          {[
            ...
            },
            {
              icon: Clock3,
              label: "Đã check-in",
              value: shiftAssignments.filter((item) => item.checkIn).length,
            },
            {
              icon: CalendarDays,
              label: "Tổng lương ca",
