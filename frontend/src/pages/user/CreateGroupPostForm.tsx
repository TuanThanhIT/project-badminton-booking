...
import { FormCreateGroupPostSchema, type FormCreateGroupPost } from "../../schemas/FormCreateGroupPostSchema";
...
const CreateGroupPostForm = ({
  ...
  const toggleWeekday = (day: number) => {
    ...
    setValue("formData.weeklySchedule.weekdays", next, { shouldValidate: true });
  };
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-lg border text-sm ${
                  checked ? "bg-sky-600 text-white border-sky-600" : "bg-white border-gray-300 text-gray-700"
                }`}
              >
                {label}
              </button>
        ...
        {errors.formData?.weeklySchedule?.weekdays && (
          <p className="text-red-500 text-sm mt-1">{errors.formData.weeklySchedule.weekdays.message}</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          {errors.formData?.weeklySchedule?.endTime && (
            <p className="text-red-500 text-sm mt-1">{errors.formData.weeklySchedule.endTime.message}</p>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        <select {...register("formData.levelWanted")} className="w-full max-w-md border rounded-md px-3 py-2">
        ...
        {errors.formData?.levelWanted && (
          <p className="text-red-500 text-sm mt-1">{errors.formData.levelWanted.message}</p>
        )}
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div className="flex items-center gap-2 mt-7">
          <input type="checkbox" {...register("formData.contact.inApp")} className="h-4 w-4" />
