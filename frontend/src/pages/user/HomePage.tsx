...
const getDiscountValue = (discount: HomeDiscount) => {
  ...
  return `${discount.value.toLocaleString("vi-VN")}d`;
};

const getDiscountApplyLabel = (applyType: string) => {
  if (applyType === "BOOKING") return "Ð?t sân";
...
const ProductCard = ({
  ...
  "
          title={loading ? "Ðang t?i s?n ph?m..." : product?.productName}
        >
          {loading ? "Ðang t?i s?n ph?m..." : product?.productName}
        </h3>
          ...
          <div className="mt-2 flex flex-wrap gap-2 text-xs font-semibold text-slate-500">
            {product?.soldCount ? (
              <span className="rounded-full bg-orange-50 px-2.5 py-1 text-orange-600">
                Ðã bán {formatNumber(product.soldCount)}
              </span>
            ) : null}
    ...
    <article onClick={onClick} className={cardClass}>
      ...
      <div className="p-5">
        <h3 className="line-clamp-1 text-xl font-extrabold text-slate-950 transition group-hover:text-sky-700">
          {loading ? "Ðang t?i..." : branch?.branchName}
        </h3>
        ...
        {!loading && (
          <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold">
            <div className="inline-flex items-center gap-1.5 rounded-full bg-sky-50 px-3 py-1.5 text-sky-700">
              ...
              <span className="text-slate-500">lu?t d?t</span>
            </div>
            ...
            <div className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1.5 text-emerald-700">
              ...
              <span className="text-slate-500">don hàng</span>
            </div>
          </div>
        )}
...
const HomePage = () => {
  ...
  const banners = homeData.banners.length
    ...
    : [
        {
          id: 1,
          title: "Chào m?ng b?n d?n v?i B-Hub",
          subtitle:
            "Ð?t sân, mua s?m và lên l?ch luy?n t?p c?u lông trong m?t tr?i nghi?m li?n m?ch.",
          imageUrl: "/img/banner4.webp",
          primaryAction: { label: "Ð?t sân ngay", href: "/branches" },
    ...
    <div className="bg-slate-50 font-sans text-slate-900">
        ...
        <div className="relative mx-auto grid min-h-[540px] max-w-[1280px] items-center gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1.35fr_0.85fr] lg:py-12">
          <div className="home-hero-copy max-w-3xl">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm font-medium text-sky-100 backdrop-blur-sm">
              <Sparkles size={16} />
              Trang ch? B-Hub - Ðam mê c?u lông b?t d?u ? dây
            </div>
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {[
                {
                  title: "Ð?t sân nhanh",
                  desc: "Ch?n sân và gi? choi ch? trong vài thao tác.",
                  icon: CalendarDays,
                },
                {
                  title: "S?n ph?m chính hãng",
                  desc: "V?t, giày và ph? ki?n c?u lông ch?t lu?ng.",
                  icon: PackageSearch,
                },
                {
                  title: "Giao d?ch minh b?ch",
                  desc: "Giá hi?n th? rõ ràng, di?u ki?n uu dãi d? hi?u.",
                  icon: ShoppingBag,
                },
                {
                  title: "H? tr? xuyên su?t",
                  desc: "Tu v?n và cham sóc khách hàng 24/7.",
                  icon: Clock,
                },
              ].map((item) => (
                <div
          ...
          <div className="home-float rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl shadow-slate-950/20 backdrop-blur">
            ...
            <div className="mt-8 grid gap-4">
              {[
                {
                  title: `${formatNumber(homeData.stats.bookingCount)} lu?t d?t sân`,
                  detail: "Các l?ch d?t sân dã thanh toán ho?c hoàn t?t.",
                  tag: "Ð?t sân",
                },
                {
                  title: `${formatNumber(homeData.stats.soldCount)} s?n ph?m dã bán`,
                  detail: "Th?ng kê t? các don hàng dã hoàn thành.",
                  tag: "Mua s?m",
                },
                {
                  title: `${formatNumber(homeData.stats.reviewCount)} dánh giá`,
                  detail: "Ph?n h?i th?c t? t? khách hàng.",
                  tag: "Ðánh giá",
                },
              ].map((item) => (
                <div
          ...
          <div className="relative mb-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-4 text-2xl font-extrabold leading-tight text-slate-950 sm:text-3xl">
                Ð?y d? sân choi, s?n ph?m và d?ch v? c?u lông
              </h2>
            </div>

            <p className="max-w-md text-sm leading-relaxed text-slate-500">
              D? li?u du?c t?ng h?p t? chi nhánh, s?n ph?m, don hàng, d?t sân và
              dánh giá khách hàng.
            </p>
          </div>
      ...
      <section className={softSectionClass}>
        <div className="mx-auto max-w-[1280px] px-4 sm:px-6">
          {renderSectionHeader({
            ...
            badge: "Danh m?c c?u lông",
            title: "Ch?n nhanh nhóm s?n ph?m yêu thích",
            desc: "Các nhóm s?n ph?m du?c trình bày b?ng hình ?nh tr?c quan, d? ch?n và d? mua.",
            buttonText: "Xem t?t c? s?n ph?m",
      ...
      <section className={sectionClass}>
        {renderSectionHeader({
          ...
          badge: "S?n ph?m m?i",
          title: "V?a v? kho theo t?ng nhóm s?n ph?m",
          desc: "Ch?n nhóm d? xem các s?n ph?m m?i du?c c?p nh?t g?n dây t?i B-Hub.",
  ...
  "
      ...
      <section className={softSectionClass}>
        <div className="mx-auto max-w-[1280px] px-4 sm:px-6">
          {renderSectionHeader({
            icon: <Flame size={15} />,
            badge: "Bán ch?y",
            title: "S?n ph?m du?c mua nhi?u",
            desc: "X?p h?ng d?a trên s? lu?ng s?n ph?m trong các don hàng dã hoàn thành.",
            buttonText: "Xem t?t c? s?n ph?m",
            onClick: () => navigate("/products"),
          })}
      ...
      {homeData.topRatedProducts.length > 0 && (
        <section className={sectionClass}>
          {renderSectionHeader({
            ...
            badge: "Ðánh giá cao",
            title: "S?n ph?m du?c khách hàng yêu thích",
            desc: "Các s?n ph?m có di?m dánh giá trung bình t?t và nhi?u ph?n h?i tích c?c.",
            buttonText: "Xem thêm",
            onClick: () => navigate("/products"),
          })}
      ...
      <section className={sectionClass}>
        {renderSectionHeader({
          ...
          badge: "Uu dãi n?i b?t",
          title: "Mã gi?m giá dành cho b?n",
          desc: "Mã du?c chia theo mua hàng, d?t sân ho?c áp d?ng cho t?t c? d?ch v?.",
          buttonText: "Mua s?m ngay",
          onClick: () => navigate("/products"),
        })}
        ...
        {activeDiscounts.length > 0 ? (
              ...
              >
                ...
                <div className="relative">
                  ...
                  <div className="mt-4 rounded-2xl border border-dashed border-sky-200 bg-sky-50/70 px-4 py-3 transition group-hover:bg-sky-50">
                    <p className="text-sm font-medium text-slate-600">Gi?m</p>
          ...
          <div className="rounded-[1.5rem] border border-dashed border-slate-200 bg-white py-14 text-center text-sm font-semibold text-slate-500">
            Chua có mã gi?m giá cho nhóm này.
          </div>
        )}
      </section>
      ...
      <section className={softSectionClass}>
        <div className="mx-auto max-w-[1280px] px-4 sm:px-6">
          {renderSectionHeader({
            icon: <MapPin size={15} />,
            badge: "Ð?t sân",
            title: "Chi nhánh du?c d?t sân nhi?u",
            desc: "G?i ý các chi nhánh có nhi?u lu?t d?t sân dã thanh toán ho?c hoàn t?t.",
      ...
      <section className={sectionClass}>
        {renderSectionHeader({
          ...
          badge: "Chi nhánh n?i b?t",
          title: "Chi nhánh du?c dánh giá t?t",
          desc: "T?ng h?p theo di?m dánh giá trung bình và s? lu?ng ph?n h?i c?a khách hàng.",
          buttonText: "Xem chi nhánh",
          onClick: () => navigate("/branches"),
        })}
        ...
        <section className={softSectionClass}>
          <div className="mx-auto max-w-[1280px] px-4 sm:px-6">
            {renderSectionHeader({
              ...
              title: "Tr?i nghi?m th?c t? t?i B-Hub",
              desc: "Các ph?n h?i tích c?c t? khách hàng v? s?n ph?m và chi nhánh.",
            })}
                ...
                >
                  ...
                  <div className="mt-5 rounded-2xl bg-slate-50 px-4 py-3 text-xs font-semibold text-slate-500">
                    {review.targetType === "BRANCH" && review.branch
                      ...
                      : review.product
                        ? `S?n ph?m: ${review.product.productName}`
                        : "B-Hub"}
                  </div>
                </article>
              ))}
        ...
        <div className="relative overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm sm:p-10 lg:p-12">
          ...
          <div className="relative mx-auto max-w-3xl text-center">
            ...
            <h2 className="mt-5 text-3xl font-extrabold leading-tight text-slate-950 sm:text-4xl">
              S?n sàng cho bu?i choi c?u lông ti?p theo?
            </h2>

            <p className="mx-auto mt-4 max-w-2xl text-sm leading-relaxed text-slate-600 sm:text-base">
              Ð?t sân nhanh, mua s?m ph? ki?n chính hãng và qu?n lý tr?i nghi?m
              c?u lông trong cùng m?t n?n t?ng B-Hub.
            </p>
