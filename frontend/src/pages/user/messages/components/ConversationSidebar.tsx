  ...
  return (
    <aside className="w-80 border-r border-gray-200 bg-white flex flex-col min-h-0">
          ...
          {(
            ...
            <button
              ...
              className={`flex-1 py-1.5 rounded-lg font-medium transition-colors ${
                tab === key ? "bg-sky-100 text-sky-800" : "text-gray-600 hover:bg-gray-50"
              }`}
            >
              {label}
            </button>
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {sorted.length === 0 ? (
          <p className="p-4 text-sm text-gray-500">Không có hội thoại phù hợp.</p>
        ) : (
          sorted.map((conversation) => {
            ...
            return (
              ...
              >
                  ...
                  <div className="flex items-start gap-2 min-w-0 flex-1">
                    <div
                      className={`w-9 h-9 rounded-full shrink-0 flex items-center justify-center text-xs font-bold ${
                        isGroup ? "bg-sky-100 text-sky-700" : "bg-gray-100 text-gray-700"
                      }`}
                    >
                      {avatarText}
                    </div>
                ...
                <p className="text-xs text-gray-500 truncate mt-1 ml-11">{lastPreview(conversation)}</p>
              </button>
            );
          })
        )}
