    ...
    def build_from_csv(self, csv_path: Path | None = None) -> dict:
        ...
        for index, row in enumerate(records, start=1):
            try:
                ...
                text_vector = self.encoder.encode_text(self._metadata_text(row))[0]
                vectors.append(weighted_average([image_vector, text_vector], [0.75, 0.25]))
                metadata.append(self._clean_metadata(row))
            except Exception as exc:
                skipped += 1
                logger.warning("Skipped product %s: %s", row.get("product_id", "unknown"), exc)

            if index == 1 or index % 10 == 0 or index == len(records):
                logger.info(
                    "Index progress: %s/%s processed, %s indexed, %s skipped",
                    index,
                    len(records),
                    len(metadata),
                    skipped,
